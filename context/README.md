...
| **Phase 11.1 (post-launch hardening)** | **+~150 lines, 5 fixes** | Three iterations to find an auth approach that survives Render's container quirks — final: `_authed_url()` embeds the token directly in the URL passed to `git fetch`/`push`, bypassing every credential helper (same as `actions/checkout`). Plus: `GIT_TERMINAL_PROMPT=0`, `db.checkpoint()` (WAL flush before `git add` — fixed the silent passcode-loss bug), `/api/_test-push` diagnostic endpoint, workflow `re-apply scrape on binary conflict` (preserves both host writes and scrape data when they race). After this wave: passcode changes, Save Draft, admin-resets, and twice-daily scrape commits all land reliably on `origin/main` as `ui:*` and `data:*` commits respectively. |
