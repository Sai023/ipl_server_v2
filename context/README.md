...
| **Phase 11.2 (season + UX polish)** | **+~50 lines, 3 fixes** | (1) `MAX_WEEKS` 8 → 10 in `base.py` + `Static/ipl_glue.js` IplConfig fallback. The W8→W9 Monday rollover was returning `season_complete:true` because `MAX_WEEKS=8` was a placeholder from before the full 10-week IPL schedule was wired up. After fix: `ui:rollover:w9` and `ui:rollover:w10` commits both landed within the same day. (2) `_pollCycle` + `_doStateRefresh` now also refresh `/api/players` on state change — without this, `ext_*` dynamic players added by the scraper after page-load appeared in selections as raw IDs (e.g. `ext_10945` instead of `Washington Sundar AR · GT`). (3) Leaderboard table on mobile: 14 columns (#/Name/W1-W10/Total/MVP) was unreadable on a 360px phone. Wrapped in `lb-scroll` with sticky #/Name and `min-width:640px` so the per-week columns swipe horizontally; desktop unchanged. End-to-end verified 2026-05-18. |
