"""
IPL Fantasy 2026 — API Route Handlers                       routes v1.0.0
===========================================================================
Phase 7 (Cleanup) — Extracted verbatim from server.py v12.8.

All 24 @app.route handlers live here, grouped by concern.
Registered on a Flask Blueprint and mounted by server.py:
    from routes import bp
    app.register_blueprint(bp)

Zero logic changes from server.py v12.8. Strict-scope extraction only.

Import safety: this module is imported by server.py AFTER db, app, and all
shared helpers are defined at module level in server.py. Python's import
system correctly resolves the partial-module reference at that point.
...
4. History   — /api/history/<n>, /api/player-points/<n>,
               /api/user-match-points/<n>, /api/debug-points/<n>
5. Save      — /api/save-next-week/<n>, /api/member/<n>, /api/match
6. Scoring   — /api/recalculate-points, /api/audit-scores/<n>,
               /api/clean-scores
7. Admin     — /api/rollover, /api/seed-history, /api/matches-status,
               /api/update-match-url
...
# ── Shared state from server.py ──────────────────────────────────────────────
# Safe circular import: server.py imports this module only AFTER defining all
# symbols below. Python's sys.modules cache provides the partially-initialised
# server module at that point, with these names already set.
from server import (                                        # noqa: E402
    db, _db_con, _log, _write_limiter, _check_rate,
    resolve_player_id, resolve_id_list, _ID_RE,
    BASE_DIR, DATA_DIR, STATIC_DIR,
    BUDGET_TOTAL, XI_SIZE, MAX_WEEKS,
)
import server as _srv   # mutable CURRENT_PUBLIC_URL lives here
...
import init_db
import tasks
from db_manager import _jloads
...
# ════════════════════════════════════════════════════════════════════════════
# 1. SYSTEM — version, ping, poll, current-week
# ════════════════════════════════════════════════════════════════════════════
...
def api_ping():
    try:
        ...
        stats.update({
            "ok": True,
            "public_url": _srv.CURRENT_PUBLIC_URL,
            "budget": BUDGET_TOTAL,
            "xi_size": XI_SIZE,
            "max_weeks": MAX_WEEKS,
        })
...
def api_resolve_player():
    ...
    try:
        ...
        if not query:
            return jsonify({"error": "query required", "code": 400}), 400
        con   = _db_con()
        match = resolve_player_id(con, query, team_hint=team)
        con.close()
        if not match:
            return jsonify({"ok": False, "error": "No match", "input": query}), 404
        tier = match.pop("_match_tier", None)
        return jsonify({
            "ok": True, "input": query, "match_tier": tier,
...
# ════════════════════════════════════════════════════════════════════════════
# 4. HISTORY / POINTS — read-only views
# ════════════════════════════════════════════════════════════════════════════
...
def api_player_points(n):
    """
    v12.7: player rows include season_pts (base) and points (cap/vc-weighted).
    ...
    try:
        ...
        with db._read() as con:
            ...
            """, (name,)).fetchall()
            if not sel_rows:
                return jsonify({"ok": True, "name": name, "total_pts": 0,
                                "players": [], "weeks": []})
            import json as _j
            latest    = sel_rows[-1]
            team_ids  = _j.loads(latest["tw_team_json"] or "[]")
            latest_cap = latest["tw_cap_id"]
            latest_vc  = latest["tw_vc_id"]
            if not team_ids:
                return jsonify({"ok": True, "name": name, "total_pts": 0,
                                "players": [], "weeks": []})
            ...
            player_rows = {
                ...
                for r in con.execute(
                    f"SELECT id,name,team,role,price,season_pts,points "
            ...
            for sel in sel_rows:
                ...
                """, ids + [wn]).fetchall()
                for r in pts:
                    mult = (CAP_MULT if r["player_id"] == cap
                            else VC_MULT if r["player_id"] == vc else 1.0)
        ...
        players_out  = []
        for pid in team_ids:
            info = player_rows.get(pid, {
                "id": pid, "name": pid, "team": "", "role": "",
                "price": 0, "season_pts": 0, "points": 0,
            })
...
def api_debug_points(n):
    try:
        ...
        with db._read() as con:
            ...
            if not sels:
                return jsonify({"ok": True, "name": n, "message": "No selections found",
                                "selections": []})
            import json as _j
            out = []; total_pts = 0
            for sel in sels:
                ...
                vc   = sel["tw_vc_id"]
                matches = con.execute(
                    "SELECT id,title,status FROM matches WHERE week_no=? ORDER BY id",
                    (wn,),
                ).fetchall()
                pmp_rows = []
                if ids:
                    ...
                    pmp_rows = con.execute(
                        ...
                        f"WHERE player_id IN ({iph}) AND week_no=?",
                        ids + [wn],
                    ).fetchall()
                ...
                out.append({
                    ...
                    "week_pts": wk_pts,
                    "matches_in_week": [{"id": m["id"], "title": m["title"],
...
# ════════════════════════════════════════════════════════════════════════════
# 5. SAVE — mutations that write selections / members / matches
# ════════════════════════════════════════════════════════════════════════════
...
def api_save_next_week(n):
    ...
    try:
        ...
        if not isinstance(team, list):
            return jsonify({"error": "team must be list", "code": 400}), 400
        rlog = []
        if team:
            con  = _db_con()
            team, rlog = resolve_id_list(con, team, display_name=n,
                                          week_no=db.get_current_week())
        ...
        if team and len(team) != XI_SIZE:
            return jsonify({"error": f"Need exactly {XI_SIZE} players (got {len(team)})",
                            "code": 422}), 422
        total_cost = 0.0
        if team:
            ...
            if not valid:
                return jsonify({"error": f"Budget exceeded: {total_cost:.1f} CR",
                                "total_cost": total_cost, "budget": BUDGET_TOTAL,
                                "code": 422}), 422
        result = db.save_next_week(n, team, cap, vc)
        return jsonify({"ok": True, "week_no": result["week_no"],
                        "total_cost": total_cost, "resolution_log": rlog})
...
def api_member(n):
    ...
    try:
        if not n or len(n) > 30:
            return jsonify({"error": "name 1-30 chars", "code": 400}), 400
        d = request.get_json(force=True, silent=True)
        if not isinstance(d, dict):
...
# ════════════════════════════════════════════════════════════════════════════
# 6. SCORING — recalculate, audit, clean
# ════════════════════════════════════════════════════════════════════════════
...
def api_recalculate_points():
    ...
    try:
        ...
        return jsonify({
            "ok": True, "rows_updated": n, "week_pts_rows": wp, "player_pts_updated": pp,
...
def api_audit_scores(n):
    """Phase 5/7: calc_pts imported from logic.scoring_engine at module level."""
    try:
        if not n or len(n) > 30:
            return jsonify({"error": "invalid name", "code": 400}), 400
        with db._read() as con:
            ...
            if not sels:
                return jsonify({"ok": True, "name": n, "weeks": [],
                                "total_stored": 0, "total_computed": 0})
            weeks_out = []
            for sel in sels:
                wn   = sel["week_no"]
                ...
                for pid in ids:
                    ...
                    pr    = con.execute(
                        "SELECT name,team,role FROM players WHERE id=?", (pid,)
                    ).fetchone()
                    ...
                    for wm in week_matches:
                        ms = con.execute(
                            ...
                            "FROM match_scores WHERE match_id=? AND player_id=?",
                            (wm["id"], pid),
                        ).fetchone()
                        if ms:
                            ...
                            match_entries.append({
                                ...
                                "suspicious": bp_ > 200,
                                "raw": {
                                    "runs": ms["runs"], "balls": ms["balls"],
                                    "fours": ms["fours"], "sixes": ms["sixes"],
                                    "overs": ms["overs"], "wickets": ms["wickets"],
                                    "runs_conceded": ms["runs_conceded"],
                                    "catches": ms["catches"], "duck": ms["duck"],
                                    "maidens": ms["maidens"], "lbw_bowled": ms["lbw_bowled"],
                                    "stumpings": ms["stumpings"],
                                },
                                "base_pts": bp_, "multiplier": mult, "final_pts": fp,
                            })
                    player_details.append({
                        "id": pid, "name": p_name,
                        "is_cap": pid == cap, "is_vc": pid == vc,
                ...
                weeks_out.append({
                    ...
                    "matches_in_week": [{"id": m["id"], "title": m["title"],
                                         "status": m["status"]} for m in week_matches],
...
def api_clean_scores():
    ...
    try:
        ...
        _log(f"[clean-scores] Cleared pmp+match_scores+ump+week_pts+season_pts. JSON: {deleted_files}")
        return jsonify({
            "ok": True,
...
# ════════════════════════════════════════════════════════════════════════════
# 7. ADMIN / ROLLOVER — rollover, seed-history, matches-status, update-match-url
# ════════════════════════════════════════════════════════════════════════════
...
def api_rollover():
    ...
    helpers + db_manager DAO methods.
    """
...
def api_seed_history():
    ...
    if re_: return re_
    try:
        init_db._auto_seed_history_if_needed()
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"error": str(e), "ok": False, "code": 500}), 500
...
def api_update_match_url():
    ...
    try:
        ...
        if not m:
            return jsonify({"error": "URL must contain a 5+ digit Cricbuzz match ID",
                            "code": 400}), 400
        cb_id     = m.group(1)
        ...
        return jsonify({
            "ok": True, "match_id": match_id, "cb_id": cb_id, "url": clean_url,
...
# ════════════════════════════════════════════════════════════════════════════
# 8. STATIC — index, static files, manifest, offline
# ════════════════════════════════════════════════════════════════════════════
