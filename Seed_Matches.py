#!/usr/bin/env python3
"""
IPL Fantasy 2026 — Match Schedule Seeder  v3.1
================================================
Multi-strategy Cricbuzz match ID discovery:
  ...
  Strategy 3: HTML regex — FIXED pattern (/live-cricket-scores/)
  Strategy 4: Official BCCI IPL 2026 schedule (74 matches, confirmed dates)

Auto-computes 'completed' count from IST match times.

Usage:
    python Seed_Matches.py                    # fully automatic
    python Seed_Matches.py --series-id 9237
    python Seed_Matches.py --completed 18     # override auto-detection
    python Seed_Matches.py --force            # re-seed and update statuses
...
MATCH_DURATION_HRS = 4  # hours after start time before match counts as complete
...
# Source: BCCI official fixture PDF + ESPNcricinfo
# ══════════════════════════════════════════════════════════════════════════════
# Format: (match_no, cricbuzz_id_or_None, "Title", "YYYY-MM-DD", "HH:MM IST")
#
# Dates confirmed from BCCI official PDFs:
#   Part 1 (M1-20):  documents.iplt20.com/bcci/documents/1773233174530_...
#   Part 2 (M21-70): documents.iplt20.com/smart-images/1774525332894_...
#
# Cricbuzz ID source: cricbuzz.com/live-cricket-scores/{ID}/slug
#   e.g. ID 149618 from /live-cricket-scores/149618/srh-vs-rcb-1st-match-...
#
# Add IDs below as matches are played. Run --force to apply.
# ──────────────────────────────────────────────────────────────────────────────
IPL_2026_SCHEDULE = [
    # ── Phase 1: Matches 1-20 (Mar 28 – Apr 12) ───────────────────────────
    (1,  "149618", "SRH vs RCB, 1st Match",   "2026-03-28", "19:30"),
    (2,  None,     "KKR vs MI, 2nd Match",    "2026-03-29", "19:30"),
    (3,  None,     "CSK vs RR, 3rd Match",    "2026-03-30", "19:30"),
    (4,  None,     "GT vs PBKS, 4th Match",   "2026-03-31", "19:30"),
    (5,  None,     "LSG vs DC, 5th Match",    "2026-04-01", "19:30"),
    (6,  None,     "SRH vs KKR, 6th Match",   "2026-04-02", "19:30"),
    (7,  None,     "CSK vs PBKS, 7th Match",  "2026-04-03", "19:30"),
    (8,  None,     "MI vs DC, 8th Match",     "2026-04-04", "15:30"),
    (9,  None,     "RR vs GT, 9th Match",     "2026-04-04", "19:30"),
    (10, None,     "SRH vs LSG, 10th Match",  "2026-04-05", "15:30"),
    (11, None,     "RCB vs CSK, 11th Match",  "2026-04-05", "19:30"),
    (12, None,     "KKR vs PBKS, 12th Match", "2026-04-06", "19:30"),
    (13, None,     "RR vs MI, 13th Match",    "2026-04-07", "19:30"),
    (14, None,     "GT vs DC, 14th Match",    "2026-04-08", "19:30"),
    (15, None,     "KKR vs LSG, 15th Match",  "2026-04-09", "19:30"),
    (16, None,     "RCB vs RR, 16th Match",   "2026-04-10", "19:30"),
    (17, None,     "PBKS vs SRH, 17th Match", "2026-04-11", "15:30"),
    ...
    # ── Playoffs (dates TBD — announced later by BCCI) ─────────────────────
...
def _merge_discovered(discovered: list) -> list:
    """Merge live-discovered IDs into schedule by position. Hardcoded IDs take priority."""
    result = []
...
def _auto_count_completed(schedule: list) -> int:
    """Count matches whose end time (start + MATCH_DURATION_HRS) has passed in IST."""
...
def _scrape_nextjs_json(html: str, debug: bool = False) -> list:
    """Extract match IDs from Cricbuzz Next.js server-side JSON blobs."""
    matches = []
    seen = set()
...
# Bug fixed: old pattern used /cricket-scores/ — Cricbuzz uses /live-cricket-scores/
# ══════════════════════════════════════════════════════════════════════════════
def _scrape_html_regex(html: str, debug: bool = False) -> list:
    matches, seen = [], set()
    for pat in [
        r'/live-cricket-scores/(\d{5,})/([^"\'<>\s\\]+)',   # primary (fixed)
        r'/cricket-scores/(\d{5,})/([^"\'<>\s\\]+)',          # legacy fallback
    ]:
        for m in re.finditer(pat, html):
            ...
            if cid not in seen:
                ...
                matches.append({"cb_match_id": cid,
                                "title": m.group(2).replace("-", " ").title()})
    if debug:
        print(f"    [regex] {len(matches)} matches")
    return matches
...
def fetch_series_matches(series_id: str, debug: bool = False) -> list:
    ...
    if nj:
        print(f"  \u2705 [Next.js JSON] {len(nj)} matches")
        return nj

    rx = _scrape_html_regex(html, debug)
    if rx:
        print(f"  \u2705 [HTML regex] {len(rx)} matches")
        return rx

    if "cf-browser-verification" in html or "Just a moment" in html:
        print("  \u26a0  Cloudflare challenge — bot detection triggered")
    else:
        print("  \u26a0  No match IDs found in page")
        if debug:
...
def seed_to_db(schedule: list, completed: int) -> None:
    ...
    for no, cid, title, date, time_ist in schedule:
        ...
        row = conn.execute("SELECT scorecard_url, status FROM matches WHERE id=?", (iid,)).fetchone()
        if row is None:
            conn.execute(
            ...
            old_url, old_st = row
            # Prefer real ID over old placeholder; always sync status
            new_url = url if cid else old_url
            if old_st != st or new_url != old_url:
                conn.execute("UPDATE matches SET status=?, scorecard_url=? WHERE id=?",
                             (st, new_url, iid))
                updated += 1
    ...
    print(f"     Updated  : {updated} status/URL changes")
    ...
    if no_id > 0:
        ...
        print(r'     Add to IPL_2026_SCHEDULE: (match_no, "ID", "Title", "YYYY-MM-DD", "HH:MM")')
    print(f"{'='*55}")
...
def verify_scorecard_url(cricbuzz_id: str) -> None:
    """Test that a scorecard URL returns parseable data."""
    ...
    try:
        ...
        print(f"  Next.js blobs    : {'\u2705' if '__next_f' in r.text else '\u274c'}")
        print(f"\n  {'PASS — scraper should work' if ok else 'FAIL — no match data (bot block or wrong ID?)'}")
    except requests.RequestException as e:
        print(f"  \u274c {e}")
...
def main():
    ...
    p.add_argument("--force",   action="store_true",
                   help="Re-seed even if data exists (updates statuses)")
    p.add_argument("--no-live", action="store_true",
                   help="Skip Cricbuzz fetch; use hardcoded schedule only")
    p.add_argument("--debug",   action="store_true")
    p.add_argument("--verify",  metavar="CB_ID",
                   help="Test a Cricbuzz scorecard URL and exit")
    args = p.parse_args()
    ...
    print("\n--- IPL 2026 Match Seeder v3.1 ---")
    (BASE_DIR / "data").mkdir(exist_ok=True)

    # Always run to update statuses, even if data exists (unless --force triggers full re-seed)
    if not args.force:
        try:
    ...
    if not args.no_live:
        ...
        if not discovered:
            print("  \u2192 Using hardcoded schedule (Match 1 confirmed: 149618)")
    else:
